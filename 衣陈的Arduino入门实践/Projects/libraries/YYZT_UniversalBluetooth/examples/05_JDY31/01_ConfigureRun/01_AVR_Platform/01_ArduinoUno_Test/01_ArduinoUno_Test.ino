/**
 * 程序功能：JDY31示例代码，配置蓝牙后运行
 * 使用教程：https://blog.yyzt.site/
 * 注意事项：1.JDY31蓝牙重新设置波特率后重新上电才会生效
...
#define bluetoothTxPin 2          // ArduinoUNO开发板的2号引脚接蓝牙模块的Tx引脚
#define bluetoothRxPin 3          // ArduinoUNO开发板的3号引脚接蓝牙模块的Rx引脚
...
#define bluetoothName "JDY31"     // 蓝牙的名称（18字符内）
...
void setup() {
  ...
  jdy31.setSerial(bluetoothSerial);                    // 设置蓝牙连接的串口
  jdy31.setAllConfigureTimeout(configureTimeout);      // 设置蓝牙总配置超时时间
  jdy31.setSingleConfigureTimeout(singleStepTimeout);  // 设置蓝牙单步配置超时时间
  // jdy31.resetBluetoothBaudrate(resetBaudrate);         // 重置蓝牙模块的波特率（如果不需要重置蓝牙模块波特率则不使用，注释即可）
  jdy31.setName(bluetoothName);          // 设置蓝牙的名称（18字符内）
  jdy31.setPassword(bluetoothPassword);  // 设置蓝牙的配对连接密码（4位数）
  jdy31.startConfigure();                // 开始配置蓝牙参数
 ...
 */
void loop() {
  if (jdy31.getConfigureState() == Configuring) {           // 如果蓝牙配置状态为正在配置中
    jdy31.configureRuning();                                // 配置蓝牙参数
    if (jdy31.getConfigureState() == ConfigureSucceeded) {  // 如果蓝牙配置状态为配置成功
#if debugState
      debugSerial.println(F("蓝牙配置成功！"));
#endif
    } else if (jdy31.getConfigureState() == ConfigureFailed) {  // 如果蓝牙配置状态为配置失败
#if debugState
