...
#define bluetoothName "HC02"      // 蓝牙的名称（16字符内）
...
void setup() {
  ...
  hc02.setSerial(bluetoothSerial);                    // 设置蓝牙连接的串口
  hc02.setAllConfigureTimeout(configureTimeout);      // 设置蓝牙总配置超时时间
  hc02.setSingleConfigureTimeout(singleStepTimeout);  // 设置蓝牙单步配置超时时间
  // hc02.resetBluetoothBaudrate(resetBaudrate);         // 重置蓝牙模块的波特率（如果不需要重置蓝牙模块波特率则不使用，注释即可）
  hc02.setName(bluetoothName);          // 设置蓝牙的名称（16字符内）
  hc02.setPassword(bluetoothPassword);  // 设置蓝牙的配对连接密码（4位数）
  hc02.startConfigure();                // 开始配置蓝牙参数
 ...
 */
void loop() {
  if (hc02.getConfigureState() == Configuring) {           // 如果蓝牙配置状态为正在配置中
    hc02.configureRuning();                                // 配置蓝牙参数
    if (hc02.getConfigureState() == ConfigureSucceeded) {  // 如果蓝牙配置状态为配置成功
#if debugState
      debugSerial.println(F("蓝牙配置成功！"));
#endif
    } else if (hc02.getConfigureState() == ConfigureFailed) {  // 如果蓝牙配置状态为配置失败
#if debugState
